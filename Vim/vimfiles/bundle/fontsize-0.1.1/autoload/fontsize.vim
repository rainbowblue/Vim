...
function! fontsize#get()
    let s:font = iconv(&guifontwide, 'gbk', &enc)
    if !strlen(s:font)
        let s:font = &guifont
    endif
...
function! fontsize#set(size)
    let s:font = &guifont
    let s:fontwide = iconv(&guifontwide, 'gbk', &enc)

    let newFont = ''
    let newFontWide = ''

    if has("gui_win32")
        let newFont = substitute(s:font, '\(.*:h\)\d\+\(:.*\)', '\1' . a:size . '\2', '')
        if strlen(s:fontwide)
            let newFontWide = substitute(s:fontwide, '\(.*:h\)\d\+\(:.*\)*',
                        \ '\1' . a:size . '\2', '')
        endif
    elseif has("gui_gtk2")
    ...
    let &guifont = newFont
    let &guifontwide = iconv(newFontWide, &enc, 'gbk')
endfunction
